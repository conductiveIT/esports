...
function tdm_core.skins.apply(player, color_hex)
    if not player or not player:is_player() then return end
    
    local meta = player:get_meta()
    local base_skin = meta:get_string("tdm_selected_skin")
...
core.register_chatcommand("skin", {
    ...
    func = function(name, param)
        ...
        elseif param:match("^#%x%x%x%x%x%x$") then
            tdm_core.skins.apply(player, param)
...
core.register_node("tdm_core:studio_light", {
    description = "Studio Light",
    drawtype = "glasslike",
    tiles = {"tdm_hud_bar.png^[colorize:#ffffff:200"},
...
for _, s in ipairs(preview_skins) do
    core.register_node("tdm_core:locker_" .. s.id, {
