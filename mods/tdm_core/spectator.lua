...
core.register_chatcommand("follow", {
    ...
    func = function(name, param)
        ...
        if param == "" then
            tdm_core.spectators.show_follow_menu(name)
            return true
        end
...
function tdm_core.spectators.show_follow_menu(name)
    local active_players = {}
    local list_items = {}
    
    -- Filter for players actually in the match (those with a team side assignment)
    if tdm_core.teams and tdm_core.teams.players then
        for pname, side in pairs(tdm_core.teams.players) do
    ...
    local fs = "formspec_version[6]size[8,10]" ..
        "background9[0,0;8,10;tdm_hud_bar.png;false;10]" ..
    ...
    core.show_formspec(name, "tdm_core:spectate_follow", fs)
end

core.register_on_player_receive_fields(function(player, formname, fields)
    if formname ~= "tdm_core:spectate_follow" then return end
    ...
    if fields.stop_follow then
        ...
        core.close_formspec(name, "tdm_core:spectate_follow")
    ...
    if fields.follower_list then
        ...
        if event.type == "CHG" or event.type == "DCL" then
            ...
            if list and list[event.index] then
                ...
                if tdm_core.spectators[name] then
                    tdm_core.spectators[name].target = target
...
core.register_on_leaveplayer(function(player)
    tdm_core.spectators[player:get_player_name()] = nil
end)
