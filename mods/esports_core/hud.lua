...
esports_core.hud.init_hud = function(player)
	...
	if esports_core.hud.player_huds[player_name] then
		for _, id in pairs(esports_core.hud.player_huds[player_name]) do
			player:hud_remove(id)
		end
	end

	esports_core.hud.player_huds[player_name] = {}
	local huds = esports_core.hud.player_huds[player_name]
	...
	if esports_core.match.is_payload then
		huds.payload_status = player:hud_add({
			type = "text",
			position = {x = 0.5, y = 0.14},
			alignment = {x = 0, y = 0},
			number = 0x00FF00,  -- Green
			text = "PAYLOAD PROGRESS: 0% | STATUS: STOPPED",
		})
	end

	if esports_core.match.is_domination then
		huds.domination_status = player:hud_add({
			type = "text",
			position = {x = 0.5, y = 0.14},
			alignment = {x = 0, y = 0},
			number = 0x00FF00,  -- Green
			text = "DOMINATION: A: NONE | B: NONE | C: NONE",
		})
	end
...
esports_core.hud.update_scores = function()
	...
	-- Update Payload HUD if needed
	if esports_core.match.is_payload then
		local progress = esports_core.payload.progress or 0
		local state = esports_core.payload.state or "STOPPED"
		local status_text = string.format("PAYLOAD PROGRESS: %d%% | STATUS: %s", progress, state:upper())

		for _, p in ipairs(core.get_connected_players()) do
			local pname = p:get_player_name()
			local huds = esports_core.hud.player_huds[pname]
			if huds and huds.payload_status then
				p:hud_change(huds.payload_status, "text", status_text)
				...
				p:hud_change(huds.payload_status, "number", color)
			end
		end
	end

	-- Update Domination HUD if needed
	if esports_core.match.is_domination and esports_core.dom and esports_core.dom.points then
		local pts = esports_core.dom.points
		local status_text = string.format("DOMINATION: A: %s | B: %s | C: %s", 
			pts.A.owner:upper(), pts.B.owner:upper(), pts.C.owner:upper())

		for _, p in ipairs(core.get_connected_players()) do
			local pname = p:get_player_name()
			local huds = esports_core.hud.player_huds[pname]
			if huds and huds.domination_status then
				p:hud_change(huds.domination_status, "text", status_text)
			end
		end
	end
	...
	if esports_core.match.is_ctf then
		...
		local b_state = esports_core.ctf.states.blue:upper()
...
-- Broadcast Feed API
function esports_core.hud.add_kill_event(killer, k_team, victim, v_team, weapon)
	...
	local icon = "esports_feed_skull.png"
	if weapon == "rifle" then icon = "esports_feed_rifle.png"
	elseif weapon == "shotgun" then icon = "esports_feed_shotgun.png"
	...
	-- Limit to 5 events
	if #esports_core.hud.kill_feed_events > 5 then
		table.remove(esports_core.hud.kill_feed_events)
	end

	esports_core.hud.refresh_spectator_feeds()
end

function esports_core.hud.refresh_spectator_feeds()
	for _, player in ipairs(core.get_connected_players()) do
		local name = player:get_player_name()
		if esports_core.is_spectator(name) then
			esports_core.hud.draw_kill_feed(player)
		else
			-- Ensure active players don't have feed artifacts
			esports_core.hud.clear_kill_feed(player)
		end
	end
end

function esports_core.hud.clear_kill_feed(player)
	local name = player:get_player_name()
	local huds = esports_core.hud.spectator_feed_huds[name]
	if huds then
		for _, ids in ipairs(huds) do
			for _, id in ipairs(ids) do player:hud_remove(id) end
		end
		esports_core.hud.spectator_feed_huds[name] = nil
	end
end

function esports_core.hud.draw_kill_feed(player)
	local name = player:get_player_name()
	esports_core.hud.clear_kill_feed(player)
	...
	esports_core.hud.spectator_feed_huds[name] = huds_list
end
...
core.register_globalstep(function(dtime)
	...
	if changed then
		esports_core.hud.kill_feed_events = new_list
		esports_core.hud.refresh_spectator_feeds()
	end
end)

function esports_core.hud.update_timer(player, time_left, state)
end

function esports_core.hud.show_intro(player, time, r_name, b_name)
	local pname = player:get_player_name()
	local huds = esports_core.hud.player_huds[pname]
	...
	if time >= 1 and time <= 5 then
		player:hud_change(huds.intro_timer_img, "text", "esports_hud_" .. time .. ".png")
	elseif time == 0 then
		player:hud_change(huds.intro_timer_img, "text", "esports_hud_go.png")
	end
end

function esports_core.hud.hide_intro(player)
	local pname = player:get_player_name()
	local huds = esports_core.hud.player_huds[pname]
	if not huds then return end
...
function esports_core.hud.show_outro(player, data)
	...
	huds.outro_bg = player:hud_add({
		...
		text = "esports_hud_bar.png^[colorize:#000000ee",
		scale = {x = -2000, y = -2000},
		alignment = {x = 0, y = 0},
	})
	...
	local win_logo = esports_core.get_team_logo(data.win_team, "esports_logo_library_dragon.png")
...
function esports_core.hud.hide_outro(player)
	...
	core.close_formspec(pname, "esports_core:outro_return")
end

esports_core.hud.update_timer = function(text, is_critical)
	for _, player in ipairs(core.get_connected_players()) do
		local pname = player:get_player_name()
		local huds = esports_core.hud.player_huds[pname]
...
esports_core.hud.update_ammo = function(player)
	...
	if ammo_list then
		for _, stack in ipairs(ammo_list) do
			...
			if name == "esports_weapons:rifle_ammo" then
...
esports_core.hud.update_scoreboard = function()
	local stats = esports_core.match.player_stats or {}
	local sorted = {}
	...
	for _, player in ipairs(core.get_connected_players()) do
		local name = player:get_player_name()
		local huds = esports_core.hud.player_huds[name]
		if huds and huds.scoreboard then
			-- Update for spectators and any player currently holding the toggle key
			if esports_core.is_spectator(name) or player_scoreboard_visible[name] then
...
esports_core.hud.toggle_scoreboard = function(player, visible)
	local name = player:get_player_name()
	...
	if huds and huds.scoreboard then
		local text = ""
		if visible then
			text = esports_core.hud.update_scoreboard()
		end
